...
VP_LOCALISATION = ROOT / "localisation" / "russian" / "ADISCORD_northern_victory_points_l_russian.yml"
...
def render_oob(tag: str, country: dict[str, object], principal_provinces: dict[int, int]) -> str:
    ...
    if tag == "MON":
        lines = [
            "division_template = {",
            '\tname = "Имперская линейная дивизия"',
            "\tregiments = {",
            *regiment_block("infantry", 6),
            ...
            "}",
            "division_template = {",
            '\tname = "Имперская пограничная бригада"',
            "\tregiments = {",
        ...
        for index in range(divisions):
            template = "Имперская линейная дивизия" if index < 9 else "Имперская пограничная бригада"
            lines.append(
    ...
    template = "Северная линейная бригада" if unit_type == "infantry" else "Северная дружина"
