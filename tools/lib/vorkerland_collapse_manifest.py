"""Shared data for the Vorkerland collapse feature and its validator."""

TAGS = (
    "TVA", "WTD", "EYR", "EGC", "RIV", "REV", "YOR", "NDN", "SWB", "VHV", "OSV", "WPA", "WPS", "PSD", "EBA", "DVA",
    "SRA", "ZTA", "SLA", "RZA", "MLR", "ERT", "IRT", "SCA",
    "TGD", "IBA", "IBL", "CSL",
)

CONTAMINATED_STATES = {
    23, 24, 49, 51, 57, 59, 60, 125, 152, 153, 154, 155,
    165, 166, 167, 168, 169, 171, 172, 173, 176, 177, 178, 180,
    181, 182, 183, 184, 185, 187, 188, 189, 190, 191, 192, 193,
    203, 204, 205, 206, 207, 208, 209, 210, 211, 212, 213, 214,
    215, 216, 217, 219, 220, 221, 222, 224,
}

# State 168 remains part of the ERT collapse partition, but starts under VAL.
DIRTY_INITIAL_OWNER_OVERRIDES = {168: "VAL"}

DIRTY_GROUPS = {
    "SLA": (49, 51, 155, 176, 187, 191),
    "RZA": (125, 177, 188, 192, 208, 213, 214, 215, 216, 217, 220),
    "MLR": (152, 153, 154, 189, 190, 219, 221, 222, 224),
    "ERT": (167, 168, 169, 171, 184, 185, 203),
    "IRT": (178, 180, 181, 182, 183, 193, 206, 207),
    "SCA": (165, 166, 172, 173, 204, 205, 209, 210, 211, 212),
}

# Zero-population corridors and wastelands owned by EXZ are not playable dirty
# states, but still have to follow an adjacent successor or EXZ survives the
# cascade. Keep this separate from DIRTY_GROUPS so the playable-state checks do
# not require manpower, supplies, or a state category here.
EXZ_REMAINDER_GROUPS = {
    "SLA": (50, 233, 329),
    "RZA": (186,),
    "ERT": (170,),
    "IRT": (330,),
}

CAPITALS = {
    "TVA": (36, 12227), "WTD": (34, 16426), "EYR": (102, 16594), "EGC": (81, 16587),
    "RIV": (306, 16643), "REV": (82, 8059), "YOR": (123, 16576),
    "NDN": (27, 16614), "SWB": (35, 16388),
    "VHV": (317, 8803), "OSV": (318, 16642),
    "WPA": (195, 8032), "WPS": (196, 7129), "PSD": (194, 2339),
    "EBA": (197, 16623), "DVA": (145, 6729), "SRA": (198, 9104),
    "ZTA": (199, 12930), "SLA": (49, 16639), "RZA": (177, 2952),
    "MLR": (152, 9806), "ERT": (169, 10693), "IRT": (181, 2226),
    "SCA": (173, 6015),
    "TGD": (105, 16589), "IBA": (90, 5135), "IBL": (91, 4007),
    "CSL": (104, 7778),
}

STATE_PARTITIONS = {
    71: {
        71: (258,339,623,672,1129,1156,1514,1980,2713,3925,4212,4771,5154,5225,5508,5691,5859,5893,6038,6295,6592,7360,7381,8101,8409,8468,8620,8901,8949,8988,9363,9988,10324,10388,10734,11454,11690,11971,12030,12241,12444,12473,12520,12733,12801,12877,16559,16561,16591),
        194: (773,1128,1375,1685,1906,1968,2192,2202,2339,2363,2571,2740,3757,4381,5469,6461,6921,7869,7989,8238,8679,8869,8881,9269,9453,9725,9908,9935,10250,10345,11082,11505,12023,12103,12302,12441),
    },
    72: {
        72: (2565,3738,4178,4262,5136,5844,6181,6636,6730,7664,7685,7959,8631,8987,9022,10896,11703,11945,12093,12433,16622,16625),
        195: (647,806,1100,1111,2896,3534,4528,4634,7090,7230,8032,9075,9149,9158,9727,10606,11635,11717,12025,12995),
        196: (236,1569,1711,3069,3151,3928,4224,4331,5207,5409,6253,6799,7129,7341,7443,7827,7934,8051,10171,10864),
    },
    74: {
        74: (2197,2402,2516,2583,3492,5408,5666,5799,6543,8497,8704,12316,12541,12947,16585),
        197: (1386,2131,2474,6560,6839,7406,10016,10251,16623),
    },
    76: {
        76: (336,2552,2866,2988,3905,5721,5741,6443,9452,9833,10964,11090,16582),
        198: (513,741,3183,4185,6510,6764,8323,8570,9104,9193,9466,11282,11411,11659,12232,12564,12584),
    },
    80: {
        80: (265,449,686,860,868,1423,1426,1581,1672,1969,2914,3083,3567,3997,4133,4278,5153,5358,5376,5676,6399,7434,7770,7819,8849,9464,9845,10034,10609,10624,10746,11472,16619,16633,16634),
        199: (531,976,1280,1889,3345,3518,4501,5314,5524,5679,5888,6456,6465,6759,6820,7267,7942,9448,9811,10990,11379,11563,11592,12508,12930),
    },
}
