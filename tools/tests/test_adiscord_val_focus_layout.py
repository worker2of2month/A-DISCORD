...
class KefreytFocusLayoutTests(unittest.TestCase):
    ...
    def test_campaign_reads_in_three_vertical_layers(self) -> None:
        ...
        # The two headline conflict routes sit above the continuation capstone.
        ...
        # Continuation begins below the capstone.
...
class KefreytWarTooltipTests(unittest.TestCase):
    ...
    def test_every_direct_war_focus_has_an_explicit_war_tooltip(self) -> None:
        expected = {
            ...
            "VAL_frontier_return_irem": "VAL_declares_war_ert_irem_tt",
            "VAL_Southern_Expansion": "VAL_declares_war_ert_south_tt",
            "VAL_Eastern_Expansion": "VAL_declares_war_irt_tt",
    ...
    def test_preparation_focuses_point_to_the_real_war_decision(self) -> None:
        ...
        start = decisions.index("VAL_cw_begin_mobilization = {")
        end = decisions.index("\n\tVAL_", start + 1)
        mobilization = decisions[start:end]
        self.assertIn("custom_effect_tooltip = VAL_cw_begin_mobilization_war_tt", mobilization)
        self.assertIn("hidden_effect = { VAL_cw_start_intervention = yes }", mobilization)

        mandate = focus_block(self.focuses, "VAL_frontier_security_plan")
        self.assertNotIn("declare_war_on", mandate)
        self.assertIn("custom_effect_tooltip = VAL_cannibal_war_route_tt", mandate)
