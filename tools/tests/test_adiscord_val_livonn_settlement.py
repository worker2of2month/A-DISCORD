...
class ValLivonnSettlement(unittest.TestCase):
    def test_kefreyt_annexes_livonn_before_contract_choice(self):
        effects = read("common/scripted_effects/ADISCORD_VAL_livonn_settlement_effects.txt")
        self.assertIn("VAL_cw_stage_livonn_settlement = {", effects)
        self.assertIn("transfer_state = 45", effects)
        self.assertIn("45 = { set_state_controller_to = VAL }", effects)
        self.assertIn("set_country_flag = VAL_cw_livonn_settlement_pending", effects)
    ...
    def test_honor_choice_returns_only_livonn_to_shabrat_side(self):
        ...
        self.assertIn("45 = { set_state_controller_to = STS }", honor)
    ...
    def test_player_gets_two_mutually_exclusive_decisions_when_contract_exists(self):
        category = read("common/decisions/categories/ADISCORD_VAL_livonn_settlement_categories.txt")
        decisions = read("common/decisions/ADISCORD_VAL_livonn_settlement_decisions.txt")
    ...
    def test_localisation_exists_in_both_languages(self):
        ru = read("localisation/russian/ADISCORD_VAL_livonn_settlement_l_russian.yml")
