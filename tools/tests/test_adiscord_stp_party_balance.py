...
class StelanderPartyBalanceContracts(unittest.TestCase):
    ...
    def test_faction_negotiations_remain_bounded_and_expensive(self) -> None:
        for faction in ("conservatives", "borons", "security", "army", "advisers", "merchants", "radicals"):
            ...
            self.assertRegex(block, r"flag\\s*=\\s*STP_pf_negotiation_cooldown\\s+value\\s*=\\s*1\\s+days\\s*=\\s*30")
        ...
        # Manual deals stop below 60 influence; the last +5 step can only cross to <65.
        # With support clamped to 100 and effects centered on 50, the strongest
        # coefficient therefore stays below 13% even at the manual-deal extreme.
        self.assertLess(0.004 * 65 * 50 / 100, 0.131)
    ...
    def test_human_party_route_opens_with_a_real_defensive_phase(self) -> None:
        ...
        self.assertIn("add_timed_idea = { idea = STP_cw_party_initial_disarray days = 35 }", begin)
    ...
    def test_bronze_congress_loss_is_a_heavy_player_party_crisis(self) -> None:
        ...
        self.assertIn("add_timed_idea = { idea = STP_cw_congress_fall_crisis days = 70 }", success)
