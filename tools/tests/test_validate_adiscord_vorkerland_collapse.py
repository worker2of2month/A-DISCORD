...
"""
    ...
    def test_main_startup_is_fresh_campaign_only(self) -> None:
        ...
        for token in (
            f"has_country_flag = {marker}",
            f"clr_country_flag = {marker}",
        ...
        self.assertLess(
            startup.find(f"has_country_flag = {marker}"),
            startup.find(f"clr_country_flag = {marker}"),
        )
        self.assertLess(
            startup.find(f"clr_country_flag = {marker}"),
            startup.find("set_global_flag = ADISCORD_vorkerland_collapse_scheduled"),
        )
        wrk_history = read("history/countries/WRK - WorkerLand.txt")
        self.assertEqual(wrk_history.count(f"set_country_flag = {marker}"), 1)
        self.assertEqual(startup.count(f"clr_country_flag = {marker}"), 1)
        self.assertEqual(startup.count("country_event = {"), 1)
        for forbidden in (
    ...
    def test_main_claimant_collapse_category_requires_local_action(self) -> None:
        ...
        for token in (
            ...
            with self.subTest(token=token):
                self.assertEqual(visible[0].count(token), 1)
    ...
    def test_every_vorkerland_superevent_route_plays_audible_sound(self) -> None:
        ...
        self.assertIn("scoped_sound_effect = superevent_vorkerland_dirty_opening_sound_e", dirty_event)
...
class BorderWarArchitectureTests(unittest.TestCase):
    ...
    def test_central_minor_wars_are_early_adjacent_wave_decision(self) -> None:
        ...
        self.assertEqual(visible.count(f"{viability_name} = yes"), 1)
        self.assertEqual(available.count(f"{viability_name} = yes"), 1)
        ...
        for target in CENTRAL_MINOR_TARGETS:
            ...
            self.assertEqual(viability.count(viability_clause), 1)
            self.assertIn(
    ...
    def test_central_wars_have_target_specific_ai_fronts(self) -> None:
        ...
        for slug, claimant in (("wkr", "WKR"), ("vad", "VAD"), ("tva", "TVA")):
            ...
            self.assertIn(f"front_control tag = {claimant}", defense)
            self.assertIn("execution_type = careful", defense)
            self.assertIn("execute_order = yes", defense)
    ...
    def test_wkr_has_an_annex_independent_home_guard_before_minor_wars(self) -> None:
        ...
        self.assertNotIn("every_country", home_guard)
        self.assertNotIn("every_state", home_guard)

        relocation = named_block(effects, "ADISCORD_vorkerland_relocate_legacy_armies")
        self.assertIn("tag = WRK", relocation)
    ...
    def test_central_winners_receive_a_short_recovery_window(self) -> None:
        ...
        for tag, winner_flag in (
            ...
            self.assertIn(f"tag = {tag}", pending)
            self.assertIn(
                f"ADISCORD_vorkerland_central_minor_winner_{winner_flag}", pending
            )
        for district in ("EYR", "EGC", "RIV", "REV", "YOR", "NDN", "SWB", "VHV", "OSV"):
            self.assertEqual(pending.count(f"{district} = {{ exists = yes"), 4)

        scheduler = named_block(
            effects, "ADISCORD_vorkerland_schedule_central_minor_settlement_recovery"
        )
        self.assertIn(
        ...
        for tag in ("WKR", "VAD", "TVA", "WRK"):
            self.assertNotIn(
                f"{tag} = {{ ADISCORD_vorkerland_schedule_central_minor_settlement_recovery = yes }}",
                startup,
            )

        sweep = named_block(
            effects, "ADISCORD_vorkerland_sweep_pending_central_minor_settlements"
        )
        for district in ("EYR", "EGC", "RIV", "REV", "YOR", "NDN", "SWB", "VHV", "OSV"):
            self.assertIn(
        ...
        self.assertIn("ADISCORD_vorkerland_focus_schedule_final_showdown = yes", first)
        self.assertIn(
            "ADISCORD_vorkerland_central_minor_settlement_controller_failed", last
        )
        self.assertIn("ADISCORD_vorkerland_focus_schedule_final_showdown = yes", last)
    ...
    def test_central_minors_can_genuinely_defeat_a_main_claimant(self) -> None:
        ...
        self.assertNotIn(
            "# A temporary main claimant defeated by a central minor",
            capitulation,
        )
    ...
    def test_tower_visual_restores_durable_state_without_duplicate_actor(self) -> None:
        effects = read("common/scripted_effects/ADISCORD_vorkerland_effects.txt")
        sync_name = "ADISCORD_vorkerland_sync_unity_tower_visual"
        sync = named_block(effects, sync_name)
        destroyed = named_block(sync, "if")
        intact = named_block(sync, "else")
        self.assertIn(
            "has_global_flag = ADISCORD_vorkerland_unity_tower_destruction_resolved",
            named_block(destroyed, "limit"),
        )
    ...
    def test_tower_settled_state_does_not_replay_explosion(self) -> None:
        ...
        self.assertIn('next_state = "ruins"', falling)
        self.assertIn("looping = no", falling)
    ...
    def test_tower_is_one_shot_and_dirty_zone_waits_three_years(self) -> None:
        ...
        for gameplay_directory in ("common", "events", "history"):
            for path in sorted((ROOT / gameplay_directory).rglob("*.txt")):
                ...
                launch_producers.extend(
                    (path.relative_to(ROOT).as_posix(), block)
                    for block in named_blocks(source, "launch_nuke")
                )
                self.assertNotIn(f"clr_global_flag = {tower_guard}", source)
        tower_launch_producers = [
            (path, block)
            for path, block in launch_producers
            if f"province = {UNITY_TOWER_PROVINCE}" in block
        ]
        self.assertEqual(
        ...
        self.assertNotRegex(
            events,
            r"(?m)^\s*id\s*=\s*ADISCORD_vorkerland_collapse\.10\b",
        )
...
class FrontAndSupplyTests(unittest.TestCase):
    ...
    def test_global_ai_preserves_native_readiness_and_supply_checks(self) -> None:
        defines = read("common/defines/ADISCORD_defines_changes.lua")
        self.assertIn("NDefines.NMilitary.PLAN_EXECUTE_RUSH = -200", defines)
        for key in (
            ...
            self.assertNotRegex(defines, rf"(?m)^\s*{re.escape(key)}\s*=")
    ...
    def test_technograd_starts_crippled_and_has_one_120_day_recovery(self) -> None:
        ...
        aid_flag = "ADISCORD_vorkerland_tgd_technical_aid_received"
        self.assertEqual(
            technical_aid.count(f"NOT = {{ has_country_flag = {aid_flag} }}"),
            1,
        )
    ...
    def test_precollapse_itoran_armistice_owns_exact_fresh_scenario_dmz(self) -> None:
        ...
        expire_armistice = named_block(
            armistice, "ADISCORD_vorkerland_expire_precollapse_itoran_armistice"
        )
        for token in (
            "tag = PWR",
            "country_exists = IVN",
            ...
            "NOT = { has_global_flag = ADISCORD_vorkerland_precollapse_itoran_armistice_expired }",
            "set_global_flag = ADISCORD_vorkerland_precollapse_itoran_armistice_active",
        ):
            self.assertIn(token, apply_armistice)
        self.assertIn(
            "has_global_flag = ADISCORD_vorkerland_collapse_started",
            expire_armistice,
        )
        self.assertIn(
            "set_global_flag = ADISCORD_vorkerland_precollapse_itoran_armistice_expired",
            expire_armistice,
        )
        self.assertIn(
            "clr_global_flag = ADISCORD_vorkerland_precollapse_itoran_armistice_active",
            expire_armistice,
        )
        for state_id in (90, 91, 93):
            applied = named_block(apply_armistice, str(state_id))
            self.assertIn("set_demilitarized_zone = yes", applied)
            self.assertIn(
                "set_state_flag = ADISCORD_vorkerland_precollapse_itoran_armistice_dmz",
                applied,
            )
            expired = named_block(expire_armistice, str(state_id))
            self.assertIn("set_demilitarized_zone = no", expired)
            self.assertIn(
                "clr_state_flag = ADISCORD_vorkerland_precollapse_itoran_armistice_dmz",
                expired,
            )
        for effect in (apply_armistice, expire_armistice):
            self.assertEqual(named_block(effect, "94"), "")
        self.assertNotIn("owns_state = 94", armistice)
    ...
    def test_itoran_armistice_is_fresh_start_and_same_tick_collapse_driven(self) -> None:
        ...
        self.assertIn(marker_guard, pwr_startup)
        self.assertIn(marker_consume, pwr_startup)
        self.assertIn(apply_call, pwr_startup)
        self.assertTrue(
            pwr_startup.find(marker_guard)
            < pwr_startup.find(marker_consume)
            < pwr_startup.find(apply_call)
        )
        pwr_history = read("history/countries/PWR - PostWarZone.txt")
        self.assertEqual(pwr_history.count(f"set_country_flag = {fresh_marker}"), 1)
        self.assertEqual(on_actions.count(marker_consume), 1)
        for forbidden in (
            "every_country",
        ...
        for state_id in (90, 91, 93):
            writers = [
                path
                for path, source in gameplay_sources
        ...
        for retired in (
            ...
            self.assertNotIn(retired, armistice + on_actions + events)
...
class CharactersAndPoliticsTests(unittest.TestCase):
    ...
    def test_zaozersk_uses_bassam_and_a_bounded_wartime_focus_program(self) -> None:
        ...
        self.assertIn("recruit_character = WRK_Bassam_Zogby", history)
        self.assertIn("promote_character = WRK_Bassam_Zogby", history)
        bassam = named_block(characters, "WRK_Bassam_Zogby")
        self.assertIn("country_leader", bassam)
        self.assertIn("ideology = pragmatism_ideology", bassam)
        self.assertIn("GFX_portrait_WRK_Bassam_Zogby", bassam)
        ...
        for token in (
            ...
            self.assertIn(token, zao_cosmetics)
        self.assertIn("ADISCORD_vorkerland_appoint_zao_administrator = yes", repair)
        self.assertIn("tree = ADISCORD_vorkerland_zao_focus", repair)
        self.assertIn("ADISCORD_vorkerland_zao_startup_repair_v1_applied", repair)
        self.assertNotIn("has_focus_tree = ADISCORD_vorkerland_zao_focus", startup)
        self.assertNotIn("ADISCORD_vorkerland_collapse.68", startup)
        self.assertNotIn("ADISCORD_vorkerland_collapse.68", monthly)
        ...
        self.assertEqual(
            set(re.findall(r"(?m)^\s*id\s*=\s*(ZAO_[a-z0-9_]+)\s*$", focus_tree)),
            expected_focuses,
        )
        self.assertEqual(len(re.findall(r"(?m)^\s*focus\s*=\s*\{", focus_tree)), 4)
        for token in (
            "tag = ZAO",
            "has_global_flag = ADISCORD_vorkerland_collapse_started",
            "has_global_flag = ADISCORD_vorkerland_collapse_wars_started",
            ...
            self.assertIn(token, focus_tree)
        for forbidden in (
            "GFX_goal_unknown",
            "declare_war_on",
            "annex_country",
            ...
            self.assertNotIn(forbidden, focus_tree)
        for focus_id in expected_focuses:
            self.assertIn(f" {focus_id}:0 ", english)
            self.assertIn(f" {focus_id}_desc:0 ", english)
            self.assertIn(f" {focus_id}: ", russian)
    ...
    def test_collapse_runtime_cannot_remove_unrelated_national_spirits(self) -> None:
        paths = (
            ...
            ("common/on_actions/01_ADISCORD_vorkerland_collapse_on_actions.txt", None),
            ("events/ADISCORD_vorkerland_events.txt", "collapse_events"),
            ("common/decisions/ADISCORD_vorkerland_decisions.txt", "collapse_decisions"),
        )
...
class InterventionAndVisualTests(unittest.TestCase):
    def test_krait_is_led_by_selevyostrov_before_and_after_puppeting(self) -> None:
        ...
        self.assertNotIn("ADISCORD_vorkerland_collapse.42", startup)
    ...
    def test_late_ivanland_and_frealor_interventions_are_one_shot_and_state_bound(self) -> None:
        ...
        self.assertNotIn(
            "IVN = { ADISCORD_vorkerland_migrate_legacy_ivanland_zaozersk_protectorate = yes }",
            startup,
        )
        self.assertNotIn(
...
class VorkerlandLocalBracketTests(unittest.TestCase):
    ...
    def test_coalition_join_splices_into_the_host_war(self) -> None:
        ...
        self.assertIn("add_to_faction = ROOT", named_block(join, "FROM"))
        self.assertLess(
            join.index("add_to_faction = ROOT"),
            join.index("ADISCORD_vorkerland_splice_into_coalition_host_war = yes"),
        )
        host = named_block(splice, "faction_leader")
        enemy = named_block(host, "every_enemy_country")
        winner = named_block(enemy, "ROOT")
        war = named_block(winner, "add_to_war")
        self.assertIn("targeted_alliance = PREV.PREV", war)
        self.assertIn("enemy = PREV", war)
        self.assertIn("single_target_only = yes", war)
        self.assertIn("NOT = { has_war_with = ROOT }", named_block(enemy, "limit"))
    ...
    def test_validator_rejects_cross_country_coalition_hosts_and_reversed_war_scopes(self) -> None:
        ...
        for before, after, expected in variants:
            with self.subTest(replacement=after):
                altered = self.effects.replace(before, after, 1)
                self.assertNotEqual(altered, self.effects)
                issues: list[str] = []
...
class ConsolidatedSourceTests(unittest.TestCase):
    def test_sections_keep_checks_isolated_without_losing_indentation(self):
        source = "# --- first ---\n\tfirst = yes\n# --- second ---\nsecond = no\n"
        self.assertEqual(source_section(source, "first"), "\tfirst = yes\n")
        self.assertEqual(source_section(source, "second"), "second = no\n")
