...
For concrete gameplay costs and rewards, prefer the native resource texticon plus the number (for example `§Y75§! £political_power_texticon`) instead of spelling out the resource name. Keep prose wording for narrative or explanatory mentions where no concrete amount is being displayed.
