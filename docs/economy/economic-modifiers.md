...
`ADISCORD_economy_calculate_army_expenses` берёт нативный `num_battalions` и сохраняет его в `ADISCORD_economy_army_battalion_count` для подсказки бюджета. Это количество батальонов, а не число шаблонов или дивизий. Пересчёт выполняется в существующем country-scoped обновлении экономики, без обхода подразделений. Нативный показатель описан в `documentation/dynamic_variables_documentation.md` установленной игры.

Батальон добавляет `0.05` к исходной месячной базе; общий множитель расходов армии `2.00` доводит ставку до `0.10`. Развитие и организация армии, война, финансирование и национальные модификаторы применяются в прежнем порядке. При обычном финансировании и без остальных поправок 10 батальонов стоят 1 в месяц; недельное списание составляет `3/13` месячной суммы. Отдельная надбавка Нодруля за число дивизий и превышение его экономического предела сохраняется. Предпросмотр финансирования использует сохранённую базу и не перезаписывает численность из последнего расчёта.

У VAL контрактное содержание добавляет `+0.25` к `VAL_contract_army_expense_factor`. Поздние реформы через `VAL_reform_mercenary_payroll` накапливают `VAL_contract_payroll_relief` шагами `-0.05` с нижним пределом `-0.25`; смена дерева фокусов не сбрасывает эту переменную. В обоих деревьях есть пять реформ, причём в дереве Совета они доступны на мирной и реваншистской ветках. При загрузке сохранения без этой переменной `VAL_migrate_mercenary_payroll` один раз учитывает записанные завершённые реформы перед пересчётом духа. Миграция не вызывается внутри награды фокуса и не начисляет скидку повторно при следующей загрузке. Авторитет и остальные скидки учитываются отдельно. Пересчёт динамического духа инвалидирует кэш экономики через `ADISCORD_economy_mark_dirty`.
...
Плоский `ADISCORD_economy_weekly_income` применяется после процентных множителей и перевода `3 / 13`. Его месячный эквивалент `13 / 3` входит в баланс для ИИ, а годовой `52` - в знаменатель долга к доходу. Доля процентов использует полный недельный доход вместе с этой прибавкой.
