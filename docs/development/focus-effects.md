...
| Усиленная дивизия | 500 казны; 35 дней, слот | Полностью оплаченная дивизия будущей армии Шабрата, максимум две. Деньги списываются при заказе; завершение добавляет готовое формирование без повторной оплаты из общих запасов. Прерывание возвращает 500 казны и освобождает слот |
| Национальный оружейный резерв | 35 PP, 1600 существующих винтовок, 28 дней, слот | Резерв Абилии достаётся её владельцу вне общего раздела. Склад с грузом в пути вмещает 9600 винтовок |
| Северная поставка | 40 PP, 2400 существующих винтовок, 21 день | Доставка разрешена, пока отправитель и канал доступны, борьба за Стеландер не завершена, а получатель воюет с NOD. Если условия утрачены, винтовки возвращаются, PP не возвращаются. Поставка влияет на настоящую войну, а не прибавляет гарантированные дни |
| Отсрочка комиссии | 35 PP, 14 дней занятого слота, повтор через 60 дней | Текущая комиссия отложена на 14 дней; этот срок конкурирует с другими операциями подготовки |
...
После `STP_cw_abila_reserve` доступны два исключающих фокуса по 35 дней, расположенных на одной строке под резервом:

| Фокус | Результат | Издержка |
|---|---|---|
| Средства военного комитета | 500 в общую казну с записью в месячные доходы действий; достаточно для отдельного заказа одной усиленной дивизии | Легитимность Шабрата -0,15; заказ требует ещё 35 дней и слот; деньги до оплаты части не закреплены за сопротивлением |
| Мастерские Абилии | Два военных завода и два места под постройки в штате 1 | Нет немедленной готовой части; до раскола производство пополняет общие склады |

Резерв можно завершить на 175-й день после развилки, фонд на 210-й, отдельный заказ дивизии на 245-й — до обычного результата около 287–289-го дня. Расчёт предполагает заранее открытый военный маршрут и доступный слот; фокус сам не создаёт часть. ИИ не начинает денежный фокус, если до выборов осталось 70 дней или меньше: резервируются полные 35 дней фокуса и 35 дней обучения. При выборе мастерских владение и контроль Абилии проверяются на протяжении фокуса. Заводы передаются с гарантированным штатом 1 через существующий раздел территории.
...
Республиканский оборонительный приказ открывает действующий `SRP_emergency_command` при настоящей войне с VAL. У остальных приказов открытие определяется `has_completed_focus`, срок бонуса — `add_timed_idea`. Все четыре решения повторяемые: `fire_only_once = no`; `days_re_enable = 42` для наступления и перегруппировки, `56` для обоих оборонительных приказов. Готовность и срок не дублируются отдельными флагами. Тексты явно называют цену, длительность и получателя; эффект по всей армии не описывается как бонус только одной провинции. ИИ запускает доступный приказ, но качество выбранного им момента требует кампании.

### Военные духи трёх сторон

Боеспособность зависит прежде всего от комплектности. Штурмовые и добровольческие шаблоны имеют нативный `priority = 2`, чтобы получать редкие орудия и винтовки раньше территориальных бригад. Это свойство загружаемого шаблона; оно не переписывает уже сохранённые шаблоны старой кампании. Состав и оплаченная цена дивизий сохраняются.

«Арсеналы военного времени» дают два завода, 2000 винтовок и 120 орудий, а также 140 дней дополнительных смен (+25% выпуска и скорости роста эффективности). Открытая закупка в военном совете стоит 35 казны и немедленно выдаёт 1600 винтовок, 60 орудий, 48 комплектов группового оружия, 30 единиц support equipment и 20 зенитных орудий. Повтор — через 42 дня. У немедленной сделки нет отложенного расчёта и нового escrow: стоимость проверяется точно, списывается один раз, после завершения мобилизации закупка закрыта. Деньги конкурируют с расширением заводов и иностранной помощью; оружие не требует собственного предварительного запаса.

При оценке цены учитывать месячный свободный бюджет, а не только накопленную казну. Например, дефицит 5600 винтовок при выпуске 8 в день означает 700 дней восстановления даже без новых потерь. Увеличение числа таких дивизий или одной только атаки не решает проблему. Наступательный приказ даёт +15% атаки, +20% прорыва и +10% скорости планирования на 21 день; его цена — командный ресурс и невозможность сменить приказ до завершения, без дополнительного ухудшения снабжения и восстановления организации.
...
Гарнизон 2/29 даёт две подготовленные бригады STS при переходе округа или успешной эвакуации кадров. Партийные гарнизоны 2/29/45 дают по две бригады, если округ остаётся STP. Цена каждой — 4000 людей и 320 винтовок. Прогноз и формирование используют одинаковые условия.
...
`STPGetFalseTrailOdds` и `STP_resolve_false_trail` используют одну шкалу подозрений. Порядок исходов: чистый след, шумный след, задержание, прорыв контрразведки. Вероятности ниже 25 — 60/30/10/0%; от 25 — 35/40/20/5%; от 50 — 15/35/35/15%; от 70 — 5/20/40/35%. Расчёт использует подозрения в конце комиссии; подсказка показывает текущую оценку.

На партийном пути подполье заранее объявляет цель. Первый цикл длится 60 дней, последующие — 28. Первый обычный ответ требует 14 + 21 день на фокусы и 21 день на проверку: 56 дней, четыре дня на реакцию. Успешная проверка снимает цель; новая не выбирается в последний день. Сначала подполье создаёт администрацию, затем гарнизон, склад за 100 казны или диверсионный запас за 240 винтовок.
...
Гарантии министрам и оборонный бюджет взаимоисключающие. Аппарат открывает управление и дополнительный слот; военная программа — завод, две оплачиваемые резервные бригады и оборону Конгресса. Общая цепочка дисциплины и президиума не должна зависеть от одной из этих ветвей. Для продолжения после исключающих путей альтернативы помещаются в один `prerequisite`.
...
Удаление dynamic modifier защищается `has_dynamic_modifier`: невыбранная ветвь могла его не получить. Снятие потребителя и очистка переменных — разные операции. Закрытие подготовки снимает подозрения, но служебная переинициализация карты в активной подготовке не должна отключать этот модификатор.
...
VAL предлагает STS 3200 винтовок за 50 казны либо те же винтовки и две временные `Kefreyt Contract Infantry` за 100. Контрактные части имеют шесть пехотных батальонов, сапёрную роту, полную комплектность и опыт 0,5; их состав и набор закреплены до удаления после войны. Полная цена VAL — 12600 людей, 4420 винтовок, 96 комплектов вооружения отделений и 60 support equipment. Свободное пополнение людьми в эти пакеты не входит.
...
Независимый победивший STS блокирует вмешательство гарантов. Иначе Нодрул и победившая партия могут вступить на сторону племени через `add_to_war` в ту же войну. До объявления войны участники объединяются в общую фракцию: major за пределами фракции племени не удерживает его от отдельной нативной конференции. Существующий совместимый союз сохраняется; для стран без фракции используется шаблон NCNS. Участник несовместимого действующего союза не переносится из него автоматически. При завершении сначала прекращаются военные отношения, затем снимаются временные major-роли и только добавленное этой кампанией членство. Созданная коалиция распускается лишь при отсутствии посторонних участников. Отдельная внешняя война не использует переменные капитуляции северного или стеландского сценария. Требуемый результат — занятие заявленных районов и 25% капитуляции партии, а при отсутствии партийного участника — Нодрула. При 65% капитуляции VAL Нодрул получает предложение перемирия. Уступка 42/55 предназначена только участвовавшей партии; нейтральный STP ничего не получает. Полное поражение от гарантов до заключения перемирия позволяет Нодрулу подчинить VAL как Допотройдск.
...
Северная ветка содержит разведку (7 дней) и исключающий выбор: пограничные свидетельства (14 дней) либо поставки (21 день). Канал поставок переносится в STS. Решение с выбором YPR/COF/TFF списывает 40 PP и 2400 винтовок. Через 21 день доставка повторно проверяет, что отправитель не капитулировал, канал сохранён, конфликт STP–STS не завершён, а выбранный получатель остаётся допустимым противником NOD. Для довоенного STP дополнительно требуется действующая подготовка. При утрате любого условия винтовки возвращаются отправителю; PP не возвращаются. Один груз может находиться в пути; `days_re_enable = 42` относится к выбранному получателю. Поставки не прибавляют дни к миссии Нодрула: они должны поддерживать настоящую боеспособность северной армии. Незавершённый довоенный груз возвращается в общий запас перед разделом; поздний callback не выдаёт его второй раз.

Во время северной войны Нодрулу доступен «Северный призыв»: 150 PP и 35 дней переводят ограниченный призыв в расширенный. Люди поступают через нативную мобилизацию населения; прямой выдачи резервистов нет. ИИ использует приказ при запасе менее 800 человек на военный завод. Завершение войны, капитуляция, потеря независимости или смена закона отменяют заказ с возвратом 150 PP; повторная проверка не позволяет понизить уже принятый закон. Одобренная и всё ещё возможная интервенция сохраняет мобилизацию между северным миром и следующим вступлением. Закрытие маршрута разрешает штатную демобилизацию при ближайшем существующем обновлении экономики.

Первые две капитуляции не завершают общую войну. Победа требует текущего поражения всех трёх противников; освобождённый участник снова блокирует соглашение. Победа NOD аннексирует COF и подчиняет YPR в её исторических владениях, оставляя TFF независимым. Поражение NOD передаёт YPR только принадлежащие NOD регионы 17/18. Внешнее завершение закрывает оставшиеся северные отношения войны без территориальной награды. Если нативная конференция успела распределить весь север, обработчик мира сверяет владельцев и контролёров всех исторических штатов коалиции. Только полный результат на стороне NOD и отсутствие других войн у северных стран разрешают восстановить независимый TFF и границы YPR; освобождённая страна или чужая оккупация блокирует этот путь. Повторный вызов после зафиксированного результата ничего не меняет. Айнхольм можно призвать отдельным приказом Нодрула, пока он остаётся его колонией; приказ добавляет его только в существующие северные войны. Их завершение выводит колонию из этих войн, сохраняя прочие конфликты.
...
Стартовые армии NOD/YPR/COF/TFF: 42/18/8/16 дивизий. Авиация NOD/STP/VAL: 500/300/800 базовых истребителей; производство, аэродромы и исходные технологии согласованы с OOB. Сопоставлять их нужно с населением, законами, шаблонами, комплектностью, снабжением и производством. Эти числа не определяют длительность войны или вероятность победы.

Послевоенный бюджет требует подтверждённого `STP_cw_postwar`; отсутствие войны во время паузы республик недостаточно. Мир SRP с VAL не завершает конфликт STP–STS, и наоборот. Объединение STS даёт cores только на его собственные исходные core-территории STP. Флот и авиация STP не копируются при разделе. Обычное вступление в альянсы, создание фракций и объявление войны остаются заблокированными; используются только скриптовые маршруты.

Шаблоны `Stelander Territorial Brigade` и `Stelander Assault Division` сохраняют фиксированный состав через `is_locked = yes`; `force_allow_recruiting = yes` оставляет обычный набор. Их цена в скриптах рассчитана на этот состав. Все пути завершения конфликта вызывают `STP_cw_finish_mobilization`: он закрывает срочные заказы, возвращает незавершённое обучение и снимает блокировку существующих шаблонов. Последующая война не открывает фиксированные заказы вновь; обычная модернизация и расширение арсеналов остаются доступны. Возврат очищает резерв до позднего завершения или отмены решения, поэтому повторный вызов не выдаёт людей и оружие второй раз.
...
Дробные запасы требуют точных границ: `NOT = { has_manpower < 4000 }`, `NOT = { has_equipment = { infantry_equipment < 320 } }`, `NOT = { command_power < 25 }`. Проверка общего оружия должна соответствовать списанию по всем производителям. Отложенный расчёт повторно проверяет ту же цену и подтверждение полного списания.
...
В игре проверить owner/controller гарантированных регионов, немедленный выбор стороны и автоматический ответ `.30` через 13 дней, комплектность армий, цены до/после оплаты, получателей помощи, оба исхода выборов, досрочное выступление и cleanup. Согласие другого игрока проверяется отдельно от переключения страны: `change_tag_from` и немедленная отправка предложения в одном эффекте могут оставить ответ ИИ.
...
| Поставки советам | STS оплачивает 25 из казны: SRP получает на склад 1200 винтовок и 30 единиц вспомогательного оснащения, на 42 дня выпуск военных заводов +20% и восстановление организации +10% |
