...
| Диверсия в тылу Нодрула | 15 PP, 240 винтовок, 28 дней | −1 инфраструктура в выбранном округе NOD и 42 дня срыва тыла. Можно вести во время гражданской войны. При активной подготовке интервенции +21 день |
...
и 600 винтовок. Недостаток ресурсов отменяет конкретную попытку, а не создаёт
...
окружное влияние и общий резерв. Баланс между внутренними партийными ячейками
не является частью этой механики. Сводка выбирается целиком через
...
Штурмовые колонны открывают повторяемый заказ: 7900 человек, 610 винтовок,
...
войны при установлении автономии. Учреждение администрации не провозглашает Содружество автоматически:
косметический тег `VAL_commonwealth` меняет отдельное решение. Иконка уровня автономии называется
...
| Боеприпасы | 2400 | 21 | 8000 винтовок, 480 орудий, 480 группового оружия, 240 поддержки, 160 ПВО |
...
| Эмигрантская бригада | 1800 | 28 | Шесть территориальных батальонов за 6000 людей и 600 винтовок |
...
за 300 и реальные 1200 винтовок/100 орудий/60 поддержки готовится 21 день.
...
VAL после собственного согласия резервирует настоящие 1200 винтовок. Первый
...
### Договоры Кефрейта и Балчанская хартия

Финальное урегулирование STP/STS/NOD сначала фиксирует всех допустимых
получателей в трёх временных переменных VAL и только затем заключает мир и
меняет автономию. `VAL_final_settlement_in_progress` закрывает повторный вход
из нативных мирных и вассальных callbacks; присоединение новых администраций
к оставшимся войнам VAL возобновляется после всего пакета. Освобождённый союзник
вновь блокирует договор. Колониальный AIN следует подтверждённому поражению
своей метрополии; независимый AIN остаётся полноценным участником проверки.
Временный major-статус снимается только с тех участников, которым его выдала
эта кампания. Ограниченный северный договор не запускает финальную кампанию.

`STP_cw_cede_ainholm_to_frontier` вызывается в области проигравшего NOD или
партийного STP. Получатель - существующий независимый TFF. Допустимы только
118 и 119, принадлежащие этой метрополии либо её колонии AIN; чужая оккупация
и нейтральные владельцы исключены. Ограниченная колония передаёт армию до
исчезновения тега. Тот же территориальный эффект принадлежит поражению NOD
в северной войне, но не её победе и не произвольному белому миру.

OCA сохраняет прежний технический тег, но называется Балчанской договорной
администрацией. Её создание загружает общий `VAL_administration_focus` с
сохранением завершённых фокусов. Двенадцать узлов `OCA_*` видны только OCA:
гражданское правление и гарнизонная директория взаимоисключаются, а портовые,
шахтёрские и снабженческие узлы остаются общими. Доходы таможни и концессий
зависят от собственного владения и контроля над 44 и 88; возвращение актива
возобновляет уже заработанный доход без повторной разовой награды.

После `VAL_Occidian_Registries` Кефрейт выбирает интеграцию либо
`VAL_Balchansk_Charter`. Хартия сохраняет OCA и даёт транзитный доход, затем
открывает общую расчётную палату. `VAL_supply_base_secured` принимает как
прямое управление 43/44/88, так и обеспеченную Балчанскую администрацию.
59/60/61 по-прежнему должны принадлежать непосредственно VAL.

Провозглашение Содружества требует `VAL_Contracts_Outlive_Kings`, действительных
целей кампании, мира, независимости и стабильности не ниже 50%. Дополнительного
требования аннексировать 29/46 поверх уже признанного соглашения с SRP нет.
Нейтральное завоевание чужой страной не засчитывается как победа Кефрейта.
Правящий Валера Солгалов получает личную черту `VAL_contract_warlord`, без
смены лидера и без повторного начисления: +5% атаки и организации, +10% скорости
планирования, -5% расхода снабжения и +0.10 командного ресурса в день.

Регрессионный контракт находится в `ValTreatyAdministrationRegressionTests`
существующего `tools/tests/test_validate_adiscord_val_rework.py`. Его модель
проверяет изменение состояния между последовательными договорами, но не
заменяет проверку нативной мирной конференции. В игре проверяются оба порядка
капитуляции NOD/STP, освобождение первого участника, активная колония AIN,
независимый AIN, третья сторона в 118/119 и сохранённая администрация OCA.
