...
По умолчанию показывать награды штатными эффектами. Обычное открытие решения в фокусе показывает только короткую строку `unlock_decision_tooltip = <id>`: цена, срок, условия и полный эффект остаются в самом решении. Не встраивать `show_effect_tooltip = yes` и `<decision>_desc` в национальный фокус. Если без контекста непонятно, зачем решение открывается, допустима одна короткая дополнительная строка. Политическая власть, стабильность, опыт, строительство и настоящие временные идеи не требуют дублирующего авторского текста.
...
Капитуляция VAL или NOD в послевоенной войне STS закрывается в `STP_pc_begin_settlement`: снимок оккупанта и возврат стеландских ядер у Кефрейта остаются авторитетными. Если Стеландер выигрывает у одного соседа, белый мир касается только этой пары. Если сам Стеландер капитулирует во время одновременно активных ультиматумных войн с VAL и NOD, settlement победителя закрывает обе внешние войны, чтобы второй ультиматум не оставался зависшим после смены режима. Победа курса освобождения сразу ставит `STP_pc_lib_val_won` или `STP_pc_lib_nod_won`; клиент, металл и подчинение остаются на поздних фокусах и их событиях.
...
Рекультивация относится к собственным исходным районам 24/42/48/54/55/56/57, имеющим `ADISCORD_vorkerland_dirty_state`, а после возврата Южного Цайгена Кефрейту также к штату 168. Исходно загрязнены 24 и 57; штат 168 получает загрязнение при распаде Воркерланда и до возврата остаётся частью внешнего спора. Три последовательных проекта для каждого доступного района стоят по 500 казны и занимают по 90 дней. Государственный депозит исключает второй заказ до завершения или возврата первого; потеря владения либо контроля возвращает платёж. Прогресс принадлежит штату: он компенсирует исходный экологический штраф и сохраняется при смене владельца. Южный Цайген можно полностью рекультивировать и затем застроить промышленными площадками, но он не является обязательным условием завершения домашней программы.
