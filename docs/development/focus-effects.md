...
| Национальный оружейный резерв | 35 PP, 16000 существующих винтовок, 28 дней, слот | Резерв Абилии достаётся её владельцу вне общего раздела. Склад с грузом в пути вмещает 96000 винтовок |
...
| Диверсия в тылу Нодрула | 15 PP, 240 винтовок, 28 дней | −1 инфраструктура в выбранном округе NOD и 42 дня срыва тыла. Можно вести во время гражданской войны. При активной подготовке интервенции +7 дней |
| Отсрочка комиссии | 35 PP, 14 дней занятого слота, повтор через 60 дней | Текущая партийная проверка округа отложена на 14 дней; этот срок конкурирует с другими операциями подготовки |
...
| Диверсия столицы | 20 PP, 360 винтовок, 35 дней, слот | Подозрения +12. Если партия удержит Фаду: −1 инфраструктура, −1 военный завод и 42 дня срыва её военного производства. Обычные операции в 28 закрыты |
| Диверсия промышленности | 10 PP, 240 винтовок, 35 дней, слот | Подозрения +10. Если партия удержит Крейден: −1 гражданский завод и те же 42 дня срыва производства. Проверка может сорвать подготовку |
...
Карта «Кому подчинится Стеландер?» красит ярким цветом только фактическую передачу: сопротивление — `STP_cw_region_goes_to_resistance`, республики — `STP_cw_region_goes_to_republics`, иначе партия. Бледный цвет показывает текущий перевес без готового перехода. Обычный округ переходит Шабрату при закреплённом влиянии и администрации, непрерывно работавшей 21 день. Легитимность строго выше 0,55 (`STP_cw_high_shabrat_legitimacy`, тот же порог сильного мандата) отдаёт спорные 2/3/29/45/46/53 Шабрату без этой администрации; соглашение Ливонина и ограниченный мятеж сохраняют свои отдельные основания. Спорные округа передаются по номеру штата, а не через обход `every_owned_state` во время изъятия. Договоры с гарнизонами 2 и 29 дают +15 влияния, а не мгновенное закрепление. После первой комиссии подозрения от 50 открывают вторую параллельную проверку; срок уже идущей комиссии не пересчитывается.
...
|---|---|---|
| Средства военного комитета | 1000 в общую казну с записью в месячные доходы действий; достаточно для двух заказов усиленной дивизии | Легитимность Шабрата -0,15; заказ требует ещё 35 дней и слот; деньги до оплаты части не закреплены за сопротивлением |
| Мастерские Абилии | Четыре военных завода и четыре места под постройки в штате 1 | Нет немедленной готовой части; до раскола производство пополняет общие склады |
...
После открытия Грязной зоны RUS идёт по намеченной ИИ ветке `RUS_focus`. Фокусы только открывают решения; войны объявляются скриптом по одному соседу. Повторный фронт закрыт эскроу `ADISCORD_vorkerland_rus_dirty_campaign_active`. Ханская война за 49/176 белым миром не засчитывает соседа и не открывает закрытую зону: `RUS_press_the_closed_zone` требует поглощения Старолесья. Если поход по Старолесью уже идёт, обработчик ханской войны не заключает сепаратный мир. Провозглашение Последней империи требует не меньше трёх удержанных соседей либо исчезновения всех шести республик зоны; награда меняет дух, ставит шовинистскую канцелярию и косметический тег `RUS_last_empire` из `cosmetic.txt` со вторым стягом.
