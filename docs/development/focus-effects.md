...
Поражение партии не переводит игрока в Нодрул и не создаёт эмигрантское
правительство. Незавершённые заказы и внутренний резерв закрываются до аннексии.
...
Помощь советников в планировании использует только положительную часть
поддержки выше 50. Их влияние отдельно уменьшает прирост политической власти:
`STP_pf_advisers_pressure = -0.002 × влияние`. Ограничение мандата за 35
политической власти передаёт армии до 5 п.п. фактического влияния советников,
повышает поддержку армии на 8 и снижает поддержку советников на 12. Оно использует
общий 30-дневный перерыв соглашений. Фокус «Условия советнической миссии»
сохраняет идентификатор `STP_ps_asylum_protocol`, но открывает внутренний выбор.
Потеря связей с Нодрулом обнуляет помощь и внешние политические издержки, сохраняя
...
фокус немедленно обнуляет помощь и политические издержки внутри награды, не полагаясь на порядок
...
## Партия: оборона, поставки и непрерывность правительства

Подготовка правительства находится в «Аппарате и подготовке», военные проекты -
в «Войне и восстановлении», помощь Нодрулу и противодействие поставкам Кефрейта -
во «Внешних операциях». Законченная выборная кампания закрывает свою категорию.
Внутренний резерв правительства не относится к внешним операциям.

Путь `STP_ps_*` дополняет партийные фракции и существующую карту подготовки:
одиннадцать довоенных, семь военных и четыре послевоенных фокуса, всего 22.
Общая ветка эмигрантского правительства, его формирования и поход возвращения
не подключаются к дереву Нодрула. Предложений сменить страну нет.
...
| Резерв правительства | 3000 | Немедленное резервирование | Одно немедленное создание временного кабинета при потере действующего правительства |
...
ресурсы дважды. Сохранённые старые эмигрантские заказы только возвращают
исходную оплату: их невидимые миссии не выдают опыт, части или новые права.

Нодрул сам соглашается на помощь. Кредит 1500 за 14 дней
...
### Внутренний резерв и удалённые заказы

«Непрерывность правительства» открывает подготовку запасных учреждений внутри
Стеландера. После завершения проекта можно зарезервировать 3000 казны, пока
Конгресс удерживается. Идентификаторы `STP_ps_evacuate_funds` и
`STP_ps_evacuated_money` сохраняются, но средства не уходят за границу.
При потере работающего правительства во время гражданской войны резерв
потребляется один раз и немедленно создаёт временный кабинет, если остался
собственный контролируемый регион. Территория не возвращается; исторические
штрафы за падение Конгресса не отменяются. Реорганизация может продолжаться.
Неиспользованный резерв возвращается отдельным решением при удержании Конгресса
либо при завершении конфликта. Квитанция очищается до выплаты.

События `ADISCORD_STP_cw.201`–`.204` сохранены как невидимые обработчики закрытия
ранее поставленных в очередь уведомлений. Старые незавершённые программы Нодрула
возвращают исходную оплату; снимки эмигрантского правительства очищаются.
Обработчики не создают страны, не перемещают персонажей и не объявляют войны.
Уже сыгранные изменения карты или выбранная игроком страна не откатываются.
...
резерв правительства и закрытие старых заказов. Матрица контрактов Кефрейта проверяет смешанных производителей,
...
первое и повторное падение Конгресса; обычная победа и поражение; старые оплаченные заказы;
игра за Шабрата и человеческий Нодрул в сетевой партии. Снимать деньги, stocks, людей, остаток
...
Приём беженцев увеличивает существующий гражданский резерв. Обучение резервирует
100 тысяч переселенцев и после 35 дней даёт 5000 добровольцев. Потеря независимости
